/*파일명: Core C Assignment 18.c

*내용 : 음원 사이트의 플레이리스트 기능을 구현하려고 한다. 이미 등록된 SONG 궂체 배열이 있을 , 전체 곡 목록을 보여주고 사용자로부터
* 플레이리스트에 추가할 노래를 번호로 선택하게 한다. 플레이리스트에 노래를 추가할 마다 플레이리스트의 곡 목록과 전체 재생 시간이
